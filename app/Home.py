...
`μ=0` にクリップされます。
    ...
    with st.expander("Monod式（基質＋酸素律速、monod_o2）"):
        st.latex(r"\mu(S, C_{O2}) = \mu_{max}\,\frac{S}{K_s+S}\cdot\frac{C_{O2}}{K_{o2}+C_{O2}}")
        st.markdown(
            """
| パラメータ | 意味 | 単位 | 典型範囲 | デフォルト |
|---|---|---|---|---|
| `mu_max` | 最大比増殖速度 | 1/h | 0.1–1.0 | 0.6 |
| `Ks` | 基質の半飽和定数 | g/L | 0.01–1.0 | 0.2 |
| `Ko2` | 溶存酸素の半飽和定数 | mg/L | 0.01–0.5 | 0.2 |

通常のMonod式に溶存酸素濃度による律速項を掛け合わせた二重Monod型です。通気・撹拌能力（kLa）が
酸素需要に追いつかない場合、実際の培養では溶存酸素濃度の低下とともに増殖速度自体が鈍化しますが、
`Monod`（基質律速のみ）は `C_O2` を一切参照しないためこの現象を表現できません。酸素律速による
増殖阻害を再現したい場合はこちらを選びます。

⚠️ **注意**: 溶存酸素 `C_O2` を状態として追跡する **kLa供給付きモデル（`OxygenWithKLa`）と
組み合わせる必要があります**。酸素モデルに需要のみモデル（`OxygenDemandOnly`）を選んだ状態で
このモデルを使おうとすると、シミュレーション実行前にエラーになります。
            """
        )
    ...
    st.markdown(
        ...
        "力学的ODEとして再現したい → **Gompertz**。基質だけでなく通気・撹拌不足による酸素律速も"
        "再現したい → **Monod式（基質＋酸素律速）**（kLa供給付き酸素モデルとの併用が必須）。"
    )
...
基質が枯渇した後もこの項自体は消費を続けようとします。ただしシミュレーター側（`_rhs`）が
`S <= 0` かつ正味の変化率が負になる場合にその変化率を0にクランプするため、実際の結果では
`S` は0で底打ちし、負にはなりません。
            """
        )
...
基質と同様、需要側（OUR、特に維持代謝項 `-mo2*X`）は `C_O2` の値に関わらず作用しようとしますが、
シミュレーター側が `C_O2 <= 0` かつ正味の変化率が負の場合に変化率を0にクランプするため、
`C_O2` も0で底打ちし負にはなりません。
            """
        )
    ...
    st.markdown(
        ...
        "溶存酸素が需要に追いつくかを見たい → **kLa供給付きモデル**。いずれの酸素モデルも、それ単体では"
        "増殖速度 `μ` 自体を溶存酸素でゲーティングしません（増殖モデル側で `MonodGrowthO2`（`monod_o2`）"
        "を選んだ場合のみ、酸素律速による増殖阻害が表現されます）。"
    )
...
with tab_numerics:
    ...
    st.markdown(
        """
- 基質(`S`)・溶存酸素(`C_O2`)は、枯渇後は変化率がゼロにクランプされ0で底打ちします。ただし
  代謝物生産モデル（Luedeking-Piret式）の非増殖連動項 `beta*X` は基質・酸素の枯渇によって
  ゲートされないため、菌体(`X`)が存在する限り生産物(`P`)は枯渇後も増え続けます（既知の簡略化）。
- 増殖速度 `μ` を溶存酸素で律速するには、増殖モデルで `MonodGrowthO2`（`monod_o2`）を明示的に
  選ぶ必要があります。既定の Monod/Logistic/Gompertz は酸素非依存のままです。
